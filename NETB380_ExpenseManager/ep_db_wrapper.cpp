...
int EP_DB_Wrapper::updateUserAccount(int aid, int newType = 0, QString newName = "", QString newDesc = "", int newCurr = 0)
{
    QSqlDatabase db = QSqlDatabase::database("appdb");
    if (db.isOpen())
    {
        ...
        if (newType > 0)
        {
            req << " type = " + QString("%1").arg(newType);
        }
        if (newName != "")
        {
             req << " name = '" + QString("%1").arg(newName) + "'";
        }
        if (newDesc > 0)
        {
            req << " description = '" + QString("%1").arg(newDesc) + "'";
        }
        if (newCurr > 0)
        {
            req << " currency = " + QString("%1").arg(newCurr);
        }
        QString rq = req.join(",");
        request.append(rq);
        request.append(" WHERE id = " + QString("%1").arg(aid));
        db.exec(request);
        if (db.lastError().isValid())
        {
            qDebug() << db.lastError().text();
 ...
 * @return
 */
int EP_DB_Wrapper::updateExpense(int expense_id, int new_aid = 0, int type = 0, double ammount = 0, int new_group_name = 0, QString new_desc = "", int dateAdded = 0, QString name = "")
{
    QSqlDatabase db = QSqlDatabase::database("appdb");
    if (db.isOpen())
    {
        ...
        if (dateAdded != 0)
        {
            req << " added_at = " + QString("%1").arg(dateAdded);
        }
        if (new_desc != "")
        {
            req << " description = '" + QString("%1").arg(new_desc) + "'";
        }       
        QString rq = req.join(",");
        request.append(rq);
